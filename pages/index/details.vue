<template>
	<div class="flex w-full items-center gap-4 justify-center">
		<div
			...
			<button
				v-else
				class="text-3xl absolute top-1/2 left-2 !cursor-not-allowed"
				disabled>
				<i class="ri-arrow-left-circle-fill"></i>
			</button>
			...
			<div
				...
				class="flex w-11/12 flex-col justify-center box-styling">
				<h1
					v-if="installationAddressToggle"
					class="md:text-3xl text-xl font-bold">
					Is the installation address same as company address ?
				</h1>
				<h1 v-else class="md:text-3xl text-xl font-bold">
			...
			<button
				v-else
				class="text-3xl absolute top-2/4 right-2 !cursor-not-allowed"
				disabled>
				<i class="ri-arrow-right-circle-fill"></i>
			</button>
		</div>
...
<script setup>
	...
	const isDataSet = () => {
		if (quoteType.value === null || quoteType.value === undefined) {
			return false;
		} else {
			switch (quoteType.value) {
				case "electric":
					if (!isMPANset(mpan.value)) return false;
					break;
				case "gas":
					if (
						mprn.value === null ||
						mprn.value === undefined ||
						mprn.value === "" ||
						isNaN(mprn.value)
					)
						return false;
		...
		if (
			installationAddress.value === null ||
			installationAddress.value === undefined
		)
			return false;
